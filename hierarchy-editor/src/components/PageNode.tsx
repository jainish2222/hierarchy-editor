...
const levelStyles: Record<PageNodeData['level'], string> = {
...
const levelLabels: Record<PageNodeData['level'], string> = {
  1: 'Level 1 · Root',
  2: 'Level 2 · Main Page',
  3: 'Level 3 · Subpage',
};

export default function PageNode({ data }: NodeProps<PageNodeData>) {
  return (
    <div className={`w-[320px] rounded-[28px] border p-5 shadow-soft ${levelStyles[data.level]}`}>
      <Handle type="target" position={Position.Top} className="!bg-slate-400" />

      <div className="mb-4 flex items-start justify-between gap-3">
        <div>
          <p className="text-[11px] font-bold uppercase tracking-[0.25em] text-slate-400">{levelLabels[data.level]}</p>
          <h3 className="mt-2 text-2xl font-semibold text-slate-900">{data.label}</h3>
        </div>
        {data.isHome ? (
          <span className="rounded-full bg-brand-600 px-3 py-1 text-xs font-semibold text-white">Editable</span>
        ) : (
          <span className="rounded-full bg-white/80 px-3 py-1 text-xs font-semibold text-slate-600 ring-1 ring-slate-200">Static</span>
        )}
      </div>
